...
public class WebSocketClientExtensionsTests
{
    ...
    {
        var client = new WebSocketClient();
        Action<HttpRequestMessage> configure = _ => { };
    ...
    [Fact]
    public void WithConfigureHandler_SetsConfigureHandler_AndReturnsSameInstance()
    {
        var client = new WebSocketClient();
        Action<SocketsHttpHandler> configure = _ => { };

        var result = client.WithConfigureHandler(configure);

        result.ShouldBeSameAs(client);
        client.Options.ConfigureHandler.ShouldBeSameAs(configure);
    }
}